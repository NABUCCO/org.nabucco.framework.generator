...
public class CommonOperationSupport {
    ...
    static MessageSend createOperation(OperationType type, Expression reciever, String property, List<Expression> params)
    ...
    static MessageSend createGetter(Expression reciever, String property) throws JavaModelException {
        return createOperation(OperationType.GETTER, reciever, property, Collections.<Expression> emptyList());
    }

    static MessageSend createGetterChain(Expression reciever, String propertyChain) throws JavaModelException {
    ...
    static MessageSend createSetterChain(Expression reciever, String propertyChain, List<Expression> arguments)
            throws JavaModelException {
        String[] tokens = propertyChain.split("\\.");
        if (tokens.length > 1) {
            // leave a last one for setter call
            reciever = createGetterChain(reciever, propertyChain.substring(0, propertyChain.lastIndexOf('.')));
        }
        reciever = createOperation(OperationType.SETTER, reciever, tokens[tokens.length - 1], arguments);
        return (MessageSend) reciever;
    }
    ...
    static BinaryExpression createNullCheck(Expression expression) throws JavaModelException {
        return JAMP.createBinaryExpression(BinaryExpressionType.EQUAL_EXPRESSION, expression, getNullLiteral(),
    ...
    static MessageSend createUpdateCall(String propertyPath) throws JavaModelException {
        ...
        Expression[] arguments = new Expression[] {
                ...
                JAMP.createSingleNameReference("oldValue"), JAMP.createSingleNameReference("newValue") };
        return JavaAstModelProducer.getInstance().createMessageSend("update", reciever, Arrays.asList(arguments));
    }

    static void addOldValueNullChecks(MessageSend updateCall, String propertyChain) throws JavaModelException {
        updateCall.arguments[1] = createNullSaveGetterChain(updateCall.arguments[1], propertyChain);
    }

    static void addNewValueNullChecks(MessageSend updateCall, String propertyChain) throws JavaModelException {
    ...
    static String formatAsPropertyIdentifier(String input) {
        ...
        StringTokenizer stringTokenizer = new StringTokenizer(input, StructuredPropertyPathEntry.SEPARATOR.toString());
