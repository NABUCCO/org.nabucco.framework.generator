 ...
 */
class NabuccoToJavaRcpViewPickerContentProviderVisitor extends NabuccoToJavaVisitorSupport implements ViewConstants {
     ...
     */
    public NabuccoToJavaRcpViewPickerContentProviderVisitor(NabuccoToJavaVisitorContext visitorContext,
    ...
    public void visit(PickerDeclaration picker, MdaModel<JavaModel> target) {
        ...
        String name = NabuccoTransformationUtility.firstToUpper(picker.nodeToken2.tokenImage) + CONTENT_PROVIDER;
     ...
     */
    private void createContentProvider(String name, AnnotationDeclaration annotation, MdaModel<JavaModel> target) {
        ...
        try {
            ...
            JavaCompilationUnit unit = super
                    .extractAst(NabuccoJavaTemplateConstants.EDIT_VIEW_PICKER_CONTENT_PROVIDER_TEMPLATE);
            TypeDeclaration type = unit
                    .getType(NabuccoJavaTemplateConstants.EDIT_VIEW_PICKER_CONTENT_PROVIDER_TEMPLATE);
            ...
            // alter field type
            FieldDeclaration viewModelField = javaFactory.getJavaAstType().getField(type, "viewModel");
            ...
            List<Argument> allArguments = javaFactory.getJavaAstMethod().getAllArguments(constructor);
            for (Argument current : allArguments) {
                current.type = modelType;
            }

            ImportReference viewModelImportRefernce = modelProducer.createImportReference(this.modelPackage
                    + '.' + this.modelName);

            javaFactory.getJavaAstUnit().addImport(unit.getUnitDeclaration(), viewModelImportRefernce);

            // select field "handler"
            FieldDeclaration handlerField = javaFactory.getJavaAstType().getField(type, HANDLER_INSTANCE);

            ((SingleTypeReference) handlerField.type).token = (name + ViewConstants.HANDLER).toCharArray();
            ...
            NabuccoAnnotation mappedFieldAnn = NabuccoAnnotationMapper.getInstance().mapToAnnotation(annotation,
