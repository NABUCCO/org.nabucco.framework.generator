 ...
 * <p/>
 * Visitor to create the fragments of jboss.xml for adapters.
 * 
 * @author Nicolas Moser, PRODYNA AG
 */
class NabuccoToXmlAdapterJBossVisitor extends NabuccoToXmlVisitorSupport implements JBossConstants {
     ...
     *            the visitor context
     */
    public NabuccoToXmlAdapterJBossVisitor(NabuccoToXmlVisitorContext visitorContext) {
    ...
    public void visit(AdapterStatement nabuccoAdapter, MdaModel<XmlModel> target) {
        ...
        try {
            XmlDocument document = this.createJBossFragment(interfaceName, adapterName);
    ...
    /**
     * Creates the JBoss fragment for a adapter.
     * 
     * @param interfaceName
     *            the interface name
     * @param adapterName
     ...
     */
    private XmlDocument createJBossFragment(String interfaceName, String adapterName) throws XmlTemplateException,
            XmlModelException {
        ...
        String adapter = NabuccoCompilerSupport.getParentComponentName(this.getVisitorContext().getPackage());

        StringBuilder jndiName = new StringBuilder();
        jndiName.append(JNDI_PREFIX);
        jndiName.append(adapter);
