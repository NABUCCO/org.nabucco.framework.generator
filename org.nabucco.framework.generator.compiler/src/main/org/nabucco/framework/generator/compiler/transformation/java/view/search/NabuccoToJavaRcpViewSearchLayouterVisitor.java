...
class NabuccoToJavaRcpViewSearchLayouterVisitor extends NabuccoToJavaVisitorSupport {
    ...
    public void visit(SearchViewStatement searchViewStatement, MdaModel<JavaModel> target) {
        ...
        String name = searchViewStatement.nodeToken2.tokenImage.replace(
                NabuccoJavaTemplateConstants.VIEW, ViewConstants.LAYOUTER);
        String projectName = super.getComponentName(NabuccoClientType.RCP);

        try {
            // Load Template
            JavaCompilationUnit unit = super
                    .extractAst(NabuccoJavaTemplateConstants.SEARCH_VIEW_LAYOUTER_TEMPLATE);
            TypeDeclaration type = unit
                    .getType(NabuccoJavaTemplateConstants.SEARCH_VIEW_LAYOUTER_TEMPLATE);
            ...
            javaFactory.getJavaAstUnit().addImport(unit.getUnitDeclaration(),
                    layoutMethod(type, searchViewStatement));
            ...
            throw new NabuccoVisitorException(
                    "Error during Java AST search view layouter modification.", jme);
        } catch (JavaTemplateException te) {
            logger.error(te, "Error during Java template datatype processing.");
            throw new NabuccoVisitorException(
                    "Error during Java template search view layouter processing.", te);
        }
    }
     ...
     */
    private ImportReference layoutMethod(TypeDeclaration type,
            SearchViewStatement searchViewStatement) throws JavaModelException {
        JavaAstElementFactory javaFactory = JavaAstElementFactory.getInstance();
        // get method
        MethodDeclaration layoutMethod = (MethodDeclaration) javaFactory.getJavaAstType()
                .getMethod(
                        type,
                        new JavaAstMethodSignature(ViewConstants.LAYOUT, new String[] {
                                ViewConstants.COMPOSITE, ViewConstants.NABUCCO_MESSAGE_MANAGER,
                                ViewConstants.TEMPLATE_SEARCH_MODEL }));
        // change parameter 3
        String name = searchViewStatement.nodeToken2.tokenImage.replace(
                NabuccoJavaTemplateConstants.VIEW, ViewConstants.LAYOUTER);
        ...
        return NabuccoToJavaRcpViewVisitorSupportUtil.getModelTypeImport(getVisitorContext(),
                searchViewStatement);
     ...
     * @throws JavaModelException
     */
    private void handleWidgetFactoryField(TypeDeclaration type, String name)
            throws JavaModelException {
        JavaAstElementFactory javaFactory = JavaAstElementFactory.getInstance();
        FieldDeclaration widgetFactoryField = javaFactory.getJavaAstType().getField(type,
                ViewConstants.WIDGET_FACTORY);
