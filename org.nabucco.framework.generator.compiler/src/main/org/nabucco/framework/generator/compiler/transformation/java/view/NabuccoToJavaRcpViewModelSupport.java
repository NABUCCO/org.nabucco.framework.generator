...
public class NabuccoToJavaRcpViewModelSupport implements NabuccoJavaTemplateConstants,
    ...
    private static final JavaAstMethodSignature SIGNATURE_GET_FIELD = new JavaAstMethodSignature(
            GET_FIELD);
    ...
    private static final JavaAstMethodSignature SIGNATURE_GET_FIELD_COMBO = new JavaAstMethodSignature(
            GET_FIELD_COMBO);

    /** Signature for setField(String str) */
    private static final JavaAstMethodSignature SIGNATURE_SET_FIELD = new JavaAstMethodSignature(
            SET_FIELD, new String[] { STRING });
    ...
    private static final JavaAstMethodSignature SIGNATURE_SET_FIELD_COMBO = new JavaAstMethodSignature(
            SET_FIELD_COMBO, new String[] { STRING });

    private static final JavaAstMethodSignature SIGNATURE_SET_MODEL_LIST = new JavaAstMethodSignature(
            SET_DATATYPE_SET, "Set");
     ...
     */
    public static List<JavaAstContainter<?>> createModelElement(
            List<NabuccoAnnotation> annotationList, JavaCompilationUnit unit,
            NabuccoToJavaRcpViewVisitorSupport util, NabuccoToJavaVisitorContext visitorContext) {
        ...
        try {
            NabuccoAnnotation mappedFieldAnn = NabuccoAnnotationMapper.getInstance()
                    .mapToAnnotation(annotationList, NabuccoAnnotationType.MAPPED_FIELD);
            ...
            if (currentMappedField.split(FIELD_SEPARATOR).length <= 2) {
                ...
                MethodDeclaration getter = (MethodDeclaration) javaFactory.getJavaAstType()
                        .getMethod(methodType, SIGNATURE_GET_FIELD);
                MethodDeclaration setter = (MethodDeclaration) javaFactory.getJavaAstType()
                        .getMethod(methodType, SIGNATURE_SET_FIELD);
                ...
                MethodDeclaration getter = (MethodDeclaration) javaFactory.getJavaAstType()
                        .getMethod(methodType, SIGNATURE_GET_FIELD_DATATYPE);

                MethodDeclaration setter = (MethodDeclaration) javaFactory.getJavaAstType()
                        .getMethod(methodType, SIGNATURE_SET_FIELD_DATATYPE);
                ...
                JavaAstContainter<TypeReference> javacontainer = util.getFieldNameToTypeReference()
                        .get(DATATYPE).get(fieldname);
                ...
                util.createSetterForFieldDatatype(visitorContext, localfield, fieldname, fieldType,
                        property);
     ...
     * @return the list of elements that needed to create a model element.
     */
    public static List<JavaAstContainter<?>> createModelElementComboBox(
            List<NabuccoAnnotation> annotationList, JavaCompilationUnit unit,
            NabuccoToJavaRcpViewVisitorSupport util) {
        ...
        try {
            ...
            MethodDeclaration getter = (MethodDeclaration) javaFactory.getJavaAstType().getMethod(
                    methodType, SIGNATURE_GET_FIELD_COMBO);
            MethodDeclaration setter = (MethodDeclaration) javaFactory.getJavaAstType().getMethod(
                    methodType, SIGNATURE_SET_FIELD_COMBO);
    ...
    public static List<JavaAstContainter<?>> createModelElementListPicker(
            List<NabuccoAnnotation> annotationList, JavaCompilationUnit unit,
            NabuccoToJavaRcpViewVisitorSupport util) {
        ...
        try {
            ...
            JavaAstContainter<FieldDeclaration> delegateField = JavaAstSupport.createField(STRING,
                    fieldName.toString(), NabuccoModifierType.PRIVATE, false);

            JavaAstContainter<MethodDeclaration> delegateGetter = JavaAstSupport
                    .createGetter(delegateField.getAstNode());
            ...
            containerList.add(NabuccoToJavaRcpViewVisitorSupportUtil
                    .createStaticFinalField(mappedField));
