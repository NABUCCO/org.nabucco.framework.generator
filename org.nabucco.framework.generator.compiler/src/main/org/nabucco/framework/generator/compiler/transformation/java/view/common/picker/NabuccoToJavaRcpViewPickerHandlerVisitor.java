 ...
 */
public class NabuccoToJavaRcpViewPickerHandlerVisitor extends NabuccoToJavaVisitorSupport implements ViewConstants,
        CollectionConstants {
     ...
     */
    public NabuccoToJavaRcpViewPickerHandlerVisitor(NabuccoToJavaVisitorContext visitorContext, String name,
     ...
     *            the target model
     */
    private void createPickerHandler(String pickerName, AnnotationDeclaration annotations, boolean isList,
        ...
        try {
            ...
            JavaCompilationUnit unit = super.extractAst(NabuccoJavaTemplateConstants.EDIT_VIEW_PICKER_HANDLER_TEMPLATE);
            TypeDeclaration type = unit.getType(NabuccoJavaTemplateConstants.EDIT_VIEW_PICKER_HANDLER_TEMPLATE);
            ...
            if (mappedFieldAnn != null && mappedFieldAnn.getValue() != null) {
                ...
                // Constructor
                JavaAstMethodSignature constructorSignature = new JavaAstMethodSignature(handlerName,
                        DATATYPE_EDIT_VIEW_MODEL);
                ...
                // import of the model
                String modelImport = pkg.replace(VIEW_PACKAGE, MODEL_PACKAGE) + PKG_SEPARATOR + model;
                importReference = producer.createImportReference(modelImport);
                javaFactory.getJavaAstUnit().addImport(unit.getUnitDeclaration(), importReference);
    ...
    private void updateForElementPicker(String type, String mappedType, MethodDeclaration method)
    ...
    private void updateForListPicker(String type, String mappedType, MethodDeclaration method)
        ...
        TypeReference typeReference = producer.createParameterizedTypeReference(SET, false, Arrays.asList(wildcard));

        IfStatement ifStatement = (IfStatement) method.statements[0];
        InstanceOfExpression condition = (InstanceOfExpression) ifStatement.condition;
        condition.type = typeReference;

        typeReference = producer.createParameterizedTypeReference(SET, false, Arrays.asList(innerType));
        ...
        Annotation annotation = producer.createAnnotation(ANNOTATION_SUPPRESS_WARNINGS, ANNOTATION_VALUE_UNCHECKED);
