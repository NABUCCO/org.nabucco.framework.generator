...
/**
 * NabuccoToXmlQueryFilterEjbJarVisitor
 * 
 * @author Nicolas Moser, PRODYNA AG
 */
class NabuccoToXmlQueryFilterEjbJarVisitor extends NabuccoToXmlVisitorSupport implements EjbJarConstants,
     ...
     *            the visitor context
     */
    public NabuccoToXmlQueryFilterEjbJarVisitor(NabuccoToXmlVisitorContext visitorContext) {
    ...
    private Element createSessionElement(XmlDocument document) throws XmlModelException, XmlTemplateException {
        ...
        session.getElementsByTagName(EJB_LOCAL).item(0).setTextContent(QF_INTERFACE + LOCAL);
        session.getElementsByTagName(EJB_REMOTE).item(0).setTextContent(QF_INTERFACE + REMOTE);
        session.getElementsByTagName(EJB_CLASS).item(0).setTextContent(QF_IMPLEMENTATION);
    ...
    private Element createPersistenceRefElement() throws XmlTemplateException {
        ...
        injectionElement.getElementsByTagName(INJECTION_TARGET_CLASS).item(0).setTextContent(QF_IMPLEMENTATION);

        injectionElement.getElementsByTagName(INJECTION_TARGET_NAME).item(0).setTextContent(QF_ENTITY_MANAGER);
