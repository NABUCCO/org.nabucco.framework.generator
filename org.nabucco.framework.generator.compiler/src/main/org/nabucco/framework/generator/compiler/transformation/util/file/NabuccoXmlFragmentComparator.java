...
class NabuccoXmlFragmentComparator implements Comparator<XmlModel> {
    ...
    public int compare(XmlModel x1, XmlModel x2) {
        ...
        int result = d1.getDocumentElement().getAttribute("type")
                .compareTo(d2.getDocumentElement().getAttribute("type"));

        if (result != 0) {
            return result;
        }

        return d1.getDocumentElement().getAttribute("order").compareTo(d2.getDocumentElement().getAttribute("order"));
    }
